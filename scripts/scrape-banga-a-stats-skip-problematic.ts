...
async function scrapeBangaAStatsSkipProblematic() {
  console.log("🚀 Starting Banga A statistics scraping (skipping problematic URLs)...");
  ...
  try {
    ...
    console.error("❌ Error during Banga A scraping:", error);
  }
}

// Run the script
if (require.main === module) {
  scrapeBangaAStatsSkipProblematic()
    .then(() => {
      console.log("✅ Banga A scraping completed");
      process.exit(0);
    })
    .catch((error) => {
      console.error("❌ Banga A scraping failed:", error);
      process.exit(1);
    });
}

...
export default function PublicSubscriptionsPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto py-8">
      <h1 className="text-2xl font-bold mb-6 text-[#0A165B]">Available Subscriptions</h1>
      <div className="space-y-6">
        {subs.map(s => (
          <div key={s.id} className="border rounded p-4 bg-white shadow">
            ...
            <div className="mb-2">Price: <span className="font-bold text-[#F15601]">{formatCurrency(s.price)}</span></div>
            <div className="mb-2">Duration: <span className="font-bold">{s.durationDays} days</span></div>
            {s.features && s.features.length > 0 && (
              <div className="mb-3">
                <h3 className="font-semibold text-sm mb-1 text-[#0A165B]">Features:</h3>
            ...
            <Link href={`/checkout?subscriptionId=${s.id}`} className="inline-block px-4 py-2 bg-[#F15601] text-white rounded hover:bg-[#E04501] font-semibold">Buy</Link>
          </div>
        ))}
      </div>
    </div>
